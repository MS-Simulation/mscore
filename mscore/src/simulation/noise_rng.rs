...
//! scheduling order.
//!
//! The m/z jitter (and, in the timsim builders, the precursor-survival draw) used
//! `rand::thread_rng`, so which thread picked up a spectrum decided its noise. That makes a run
//! irreproducible, and it is not cosmetic: a jittered m/z can cross a TOF bin boundary, which
//! changes the `(scan, tof)` dedup and so the peaks written to the file.
//!
...
//!
//! Ported from rustims PR #486 (`mscore::simulation::noise_rng`) so the federated crates carry the
//! same derivation and the same stream keys.
